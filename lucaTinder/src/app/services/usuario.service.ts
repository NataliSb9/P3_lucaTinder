...
export class UsuarioService {
  ...
  convertirAUsuario(resp: any): Usuario {
    ...
    let usuario: Usuario = new Usuario(resp._id,resp.name, resp.genero, resp.email,resp.age,resp.descripcion ,resp.gustos, resp.arrLikes, resp.arrDislike,resp.prefGen, resp.foto)
  ...
  convertirAUsuarios(resp: any[]): Usuario[] {
    ...
    for (let i = 0; i < resp.length; i++) {
      let usuario: Usuario = new Usuario(resp[i]._id,resp[i].name, resp[i].genero, resp[i].email,resp[i].age,resp[i].descripcion ,resp[i].gustos, resp[i].arrLikes, resp[i].arrDislike,resp[i].prefGen, resp[i].foto);
